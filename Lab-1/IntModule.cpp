...
int MultARR(vector<int>& ARR, int start, int end) {
    int mult{ 1 };
...
bool Daip(int i, int start, int end) {
...
int CountNIGGAtive(vector<int>& ARR) {
    int count{ 0 };
    for (int i = 0; i < static_cast<int>(ARR.size()); i++) {
        if (ARR[i] < 0)count++;
    }
...
int CountMultN(vector<int>& ARR, int factor) {
    int count{ 0 };
    for (int i = 0; i < static_cast<int>(ARR.size()); i++) {
        if (ARR[i] % factor == 0)count++;
    }
    return count;
}
